'''
Priority Queues, as the name suggests, are queues where the elements have different priorities. It does not always follow the FIFO rule (e.g. people waiting in line and VIPs arrive).
...
- heappop(heap) : This function removes and returns the smallest element from heap. Again, the order is adjusted, so as heap structure is maintained.
...
- heapreplace(heap, ele) : This function also inserts and pops element in one statement, but in this, minimum element is first popped, then the new element is pushed.
